...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    plt.xlabel('Date')
    plt.title('Number of orders by date and holidays (vertical light blue dotted lines)')
    for date_holiday, bool_holiday in zip(df['holiday'].index,
                                          df['holiday'].values):
        if bool_holiday:
            plt.axvline(x=date_holiday, ls='dotted',color=(0.2, 0.5, 1))
    plt.show()